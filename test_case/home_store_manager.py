...
class HomeStoreManagerTest(myunit.ErpTest):
    ''' 店铺经理主页 '''
    ...
    def setUpClass(self):
        ...
        self.home = homepage.Homepage( self.driver, user_info.STORE_MANAGER )
        store_manager = api_option.UserApi(user_info.STORE_MANAGER)
        self.my_stats = store_manager.get_my_stats()
        self.new_orders = api_option.OrderApi().get_orders(token=store_manager.get_token(), page=1, per_page=5, order='updated_at%20desc')['data']
        self.status = user_info.STORE_MANAGER_STATUS
    ...
    def test_5_employed_count(self):
        ''' 店铺经理：销售数量 '''
        self.assertEqual( self.home.get_count('employed_count'), str(self.my_stats['employed_count']))
