...
class CloseTest(myunit.ErpTest):
    ...
    def setUpClass(self):
        ...
        self.test_order.execute('receive')
        login.Login().user_login(self.driver, const.ACCOUNTANT)
    ...
    def test__12_before_freight(self):
        ''' 操作前订单详情: 实际垫资金额 '''
    ...
    def test__16_check(self):
        ''' 发送完成结算请求 '''
        random_values = { 'payment_images': '/Users/iced_me/Desktop/Work/images/07.jpg' }
        self.test_order.close_send_value( self.driver, random_values )
        self.test_order.check(self.driver, 'close')
        message_element = self.driver.find_element_by_xpath(const.STATUS_MESSAGE)
        self.assertEqual(message_element.get_attribute('innerHTML'), '订单已完成')
