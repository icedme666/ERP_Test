...
class HomeCapitalManagerTest(myunit.ErpTest):
    ''' 资金经理主页 '''
    ...
    def setUpClass(self):
        ...
        self.home = homepage.Homepage( self.driver, user_info.CAPITAL_MANAGER )
        capital_manager = api_option.UserApi(user_info.CAPITAL_MANAGER)
        self.my_stats = capital_manager.get_my_stats()
        self.new_orders = api_option.OrderApi().get_orders(token=capital_manager.get_token(), page=1, per_page=5, order='updated_at%20desc')['data']
        self.status = user_info.CAPITAL_MANAGER_STATUS
